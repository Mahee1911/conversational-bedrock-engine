...
from settings.app_config import AWS_PROFILE, DEFAULT_MODEL_ID
from schemas.chat_models import AgentConfig
from strands.agent.conversation_manager import SlidingWindowConversationManager
from strands.session.s3_session_manager import S3SessionManager


def create_agent(thread_id: str, model: BedrockModel, conversation_manager, s3_session_manager_config: dict,
                 agent_instructions: str, tools: list) -> Agent:
    """Restore agent state from S3 with improved error handling"""
    try:
        print(
            f"thread_id: {thread_id}, "
            f"s3 bucket name for history: {s3_session_manager_config.get('bucket_name')}"
        )
